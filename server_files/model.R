...
output$contentsNmlMsg <- renderPrint({
  ...
  if (nmc==0) {
    cat('The project has not control file !')
  }
  else {
...
output$irradParams <- renderPrint({
  ...
  wavelength = sp[,1]
  fullSpectrumRange = range(wavelength)
  photonFlux = sp[,2]

  # Select in range
  selw = wavelength >= input$spectrumRange[1] &
         wavelength <= input$spectrumRange[2]
  wavelength = wavelength[selw]
  photonFlux = photonFlux[selw]

  # Rescale if adequate
  if(beamIntensity >= 0)
    photonFlux = photonFlux/sum(photonFlux) * beamIntensity
  ...
  # Update reacProc
  reacProc[['wavelength']] <<- wavelength
  reacProc[['photonFlux']] <<- photonFlux

})
output$irradSpectrum <- renderPlot({
  if(is.null(reacProc$photonFlux))
    return(NULL)
  ...
  plot(
    reacProc$wavelength, reacProc$photonFlux,
    type ='l', col = cols[5],
    xlab = 'Wavelength / nm',
